...
- (void) addDoubleTap {
  ...
  NSLog(@"DOUBLE TAP");
  UITapGestureRecognizer *doubleTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(onTapToggleButton)];
...
-(void)composeInterface {
    ...
    if (_captureManager) {
        ...
        _cameraShutter.center = CGPointMake(self.center.x, self.center.y*1.85);
...
- (void)onTapDismissButton {
  ...
  [self cameraSessionManagerDidTapDismiss];
...
- (void) cameraSessionManagerDidTapDismiss
{
  if (self.delegate)
  {
    if ([self.delegate respondsToSelector:@selector(didTapDismiss)]) {
